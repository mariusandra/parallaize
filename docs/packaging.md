...
```bash
sudo apt install ./artifacts/packages/parallaize_0.3.0-1_amd64.deb
...
The clean QEMU-host replay installed `parallaize_0.3.0-1_amd64.deb`, kept `incus.socket` active and enabled, exposed `/usr/libexec/incus/incusd` as the only `incusd` process, reported provider `hostStatus=ready`, started the packaged Caddy unit, and passed the packaged smoke workflow end to end:
